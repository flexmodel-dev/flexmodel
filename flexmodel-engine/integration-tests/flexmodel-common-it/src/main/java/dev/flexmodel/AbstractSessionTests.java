...
import dev.flexmodel.entity.Classes;
import dev.flexmodel.entity.PersonEntity;
import dev.flexmodel.entity.Student;
import dev.flexmodel.entity.StudentDetail;
...
import static dev.flexmodel.query.Direction.DESC;
import static dev.flexmodel.query.Query.*;
...
public abstract class AbstractSessionTests {
  ...
  private void testOneToOneRelation(String studentEntityName, String studentDetailEntityName) {
    ...
    Map<String, Object> oneToOne = session.data().findAll(studentEntityName, query).getFirst();
  ...
  private void testOneToManyRelation(String classesEntityName, String studentEntityName) {
    ...
    List<Map<String, Object>> oneToMany = session.data().findAll(classesEntityName, query);
  ...
  private void testInsertBasicRecord(String entityName) {
    ...
    // 验证插入的数据
    Map<String, Object> insertedRecord = session.data().findById(entityName, record.get("id")).orElse(null);
  ...
  void testInsertBatch() {
    // 场景1：不提供 ID，由数据库自动生成
    testInsertBatchWithAutoGeneratedId();
    // 场景2：显式提供 ID
    testInsertBatchWithExplicitId();
  }

  /**
   * 批量插入 - 不指定 ID，验证自动生成 ID 且数据正确
   */
  private void testInsertBatchWithAutoGeneratedId() {
    String entityName = generateEntityName("testInsertBatchAutoId");
    createTeacher2(entityName);
    registerEntity(entityName);
    ...
    for (int i = 1; i <= 5; i++) {
      Map<String, Object> record = new HashMap<>();
      record.put("name", "自动生成教师" + i);
      record.put("age", 20 + i);
      record.put("description", "自动ID测试描述" + i);
      record.put("birthday", LocalDate.of(1990, i, 1));
      records.add(record);
    }

    int affectedRows = session.data().insertAll(entityName, records);
    Assertions.assertEquals(5, affectedRows, "应该插入5条记录");

    // 验证 ID 被回填
    for (int i = 0; i < records.size(); i++) {
      Object id = records.get(i).get("id");
      Assertions.assertNotNull(id, "第" + (i + 1) + "条记录的 ID 应该被自动回填");
    }

    // 验证数据可以从数据库读回
    List<Map<String, Object>> found = session.data().findAll(entityName,
        Query.Builder.create().where(Expressions.field("name").contains("自动生成教师")).build());
    Assertions.assertEquals(5, found.size(), "应查到5条自动ID记录");
  }

  /**
   * 批量插入 - 显式指定 ID，验证走 JDBC batch 路径且数据正确
   */
  private void testInsertBatchWithExplicitId() {
    String entityName = generateEntityName("testInsertBatchExplicitId");
    createTeacher2(entityName);
    registerEntity(entityName);
    ...
    for (int i = 1; i <= 5; i++) {
      Map<String, Object> record = new HashMap<>();
      record.put("id", (long) (1000 + i)); // 显式指定 ID
      record.put("name", "显式ID教师" + i);
      record.put("age", 30 + i);
      record.put("description", "显式ID测试描述" + i);
      record.put("birthday", LocalDate.of(1985, i, 15));
      records.add(record);
    }
    ...
    // 验证指定的 ID 可以被正确查到
    for (int i = 1; i <= 5; i++) {
      Map<String, Object> found = session.data().findById(entityName, 1000 + i).orElse(null);
      Assertions.assertNotNull(found, "ID=" + (1000 + i) + " 的记录应存在");
      Assertions.assertEquals("显式ID教师" + i, found.get("name"));
      Assertions.assertEquals(30 + i, found.get("age"));
    }
  }
  ...
  void testFindById() {
    ...
    createTeacher2(entityName);
    Map<String, Object> record = session.data().findById(entityName, 1).orElse(null);
    Assertions.assertFalse(record.isEmpty());
    Optional<Map<String, Object>> record2 = session.data().findById(entityName, 999);
    Assertions.assertFalse(record2.isPresent());
  }
  ...
  void testFind() {
    ...
    List<Map<String, Object>> list = session.data().findAll(entityName, query);
    Assertions.assertFalse(list.isEmpty());
  ...
  void testFindByQueryConditionWhenHasGroupBy() {
    ...
    // 聚合分组
    List<Map<String, Object>> groupList = session.data().findAll(entityName, query -> query
    ...
    // group by time
    List<Map<String, Object>> dateFormatList = session.data().findAll(entityName, query -> query
      .select(projection -> projection
    ...
    List<Map<String, Object>> dateFormatList1 = session.data().findAll(entityName, query -> query
      .select(projection -> projection
    ...
    List<Map<String, Object>> dateFormatList2 = session.data().findAll(entityName, query -> query
      .select(projection -> projection
    ...
    List<Map<String, Object>> dayOfYearList = session.data().findAll(entityName, query -> query
      .select(projection -> projection
    ...
    List<Map<String, Object>> dayOfMonthList = session.data().findAll(entityName, query -> query
      .select(projection -> projection
    ...
    List<Map<String, Object>> dayOfWeekList = session.data().findAll(entityName, query -> query
      .select(projection -> projection
  ...
  void testFindAll() {
    ...
    createTeacher2(entityName);
    List<Map<String, Object>> list = session.data().findAll(entityName, createSimpleQuery());
    Assertions.assertFalse(list.isEmpty());
  }
  ...
  void testFindAllWhenResultDto() {
    ...
    createTeacher2(entityName);
    List<Map<String, Object>> mapList = session.data().findAll(entityName, createSimpleQuery());
