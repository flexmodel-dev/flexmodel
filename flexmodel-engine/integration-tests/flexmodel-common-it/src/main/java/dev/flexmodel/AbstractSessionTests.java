...
public abstract class AbstractSessionTests {
  ...
  void testInsertAll() {
    // 场景1：不提供 ID，由数据库自动生成
    testInsertAllWithAutoGeneratedId();
    // 场景2：显式提供 ID
    testInsertAllWithExplicitId();
  }

  /**
   * 批量插入 - 不指定 ID，验证自动生成 ID 且数据正确
   */
  private void testInsertAllWithAutoGeneratedId() {
    String entityName = generateEntityName("testInsertAllAutoId");
    createTeacher2(entityName);
    registerEntity(entityName);
    ...
    // 验证数据可以从数据库读回
    List<Map<String, Object>> found = session.data().find(entityName,
   ...
   */
  private void testInsertAllWithExplicitId() {
    String entityName = generateEntityName("testInsertAllExplicitId");
    createTeacher2(entityName);
    registerEntity(entityName);
    ...
    // 验证指定的 ID 可以被正确查到
    for (int i = 1; i <= 5; i++) {
      Map<String, Object> found = session.data().findById(entityName, 1000 + i);
