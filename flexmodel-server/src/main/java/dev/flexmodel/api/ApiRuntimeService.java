...
import dev.flexmodel.settings.consumer.SettingsEventConsumer;
import dev.flexmodel.api.dto.LogStatResponse;
import dev.flexmodel.common.dto.PageDTO;
import dev.flexmodel.api.execution.ExecutionStrategy;
import dev.flexmodel.api.execution.ExecutionStrategyFactory;
...
import dev.flexmodel.common.matchers.UriTemplate;
import dev.flexmodel.common.utils.JsonUtils;
import dev.flexmodel.common.utils.PatternMatchUtils;
...
@ApplicationScoped
public class ApiRuntimeService {

  private static final Logger log = org.slf4j.LoggerFactory.getLogger(ApiRuntimeService.class);
  @Inject
    ...
    for (ApiDefinition apiDefinition : apis) {
      ...
      if (pathParameters != null && method.equals(apiDefinition.getMethod())) {
        ...
        if (isAuth) {
          ...
          checkToken(projectId, routingContext, identityProvider);
        }

        ApiDefinitionMeta.Execution execution = meta.getExecution();

        RequestScriptContext httpScriptContext = buildHttpScriptContext(projectId, routingContext);
        try {
          // 执行具体类型的请求
  ...
  private RequestScriptContext buildHttpScriptContext(String projectId, RoutingContext routingContext) {
    RequestScriptContext requestScriptContext = new RequestScriptContext(projectId);

    RequestScriptContext.Request request = new RequestScriptContext.Request(routingContext.request().method().name(),
  ...
  public void checkToken(String projectId, RoutingContext routingContext, String providerName) {
