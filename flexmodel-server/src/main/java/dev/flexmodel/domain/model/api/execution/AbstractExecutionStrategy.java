...
public abstract class AbstractExecutionStrategy implements ExecutionStrategy {
  ...
  @Override
  public void execute(ApiDefinition apiDefinition, ApiDefinitionMeta.Execution execution, Map<String, String> pathParameters, RequestScriptContext httpScriptContext) {
    // 前置脚本
    ...
    // 设置返回值
    httpScriptContext.setResponse(new RequestScriptContext.Response(200, "OK", new HashMap<>(), resMap));
    // 后置脚本
    postExecute(execution, httpScriptContext);
  }

  protected void preExecute(ApiDefinitionMeta.Execution execution, RequestScriptContext httpScriptContext) {
    ...
    if (preScript != null) {
      try {
        ...
        JavaScriptUtil.execute(preScript, Map.of(RequestScriptContext.SCRIPT_CONTEXT_KEY, contextMap));
  ...
  protected void postExecute(ApiDefinitionMeta.Execution execution, RequestScriptContext httpScriptContext) {
    // 执行后置脚本
    String postScript = execution.getPostScript();
    if (postScript != null) {
      try {
        Map<String, Object> contextMap = httpScriptContext.buildContextMap();
        JavaScriptUtil.execute(postScript, Map.of(RequestScriptContext.SCRIPT_CONTEXT_KEY, contextMap));
        httpScriptContext.syncFromMap(contextMap);
      } catch (Exception e) {
