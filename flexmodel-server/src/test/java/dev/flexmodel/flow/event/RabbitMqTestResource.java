...
/**
 * Testcontainers 启动 RabbitMQ broker，并向 Quarkus 注入连接配置与启用 events-out 通道。
 * <p>
 ...
 * （{@code mp.messaging.outgoing.events-out.host/port/username/password}）。
...
public class RabbitMqTestResource implements QuarkusTestResourceLifecycleManager {
  ...
  public Map<String, String> start() {
    ...
    config.put("mp.messaging.outgoing.events-out.host", host);
    config.put("mp.messaging.outgoing.events-out.port", String.valueOf(port));
    config.put("mp.messaging.outgoing.events-out.username", username);
    config.put("mp.messaging.outgoing.events-out.password", password);
    // 显式提供连接配置后 DevServices 自动旁路；此处再确认禁用，避免无 Docker 时误触
    config.put("quarkus.rabbitmq.devservices.enabled", "false");
    // 启用 events-out 通道，使桥接真正转发到 broker
    config.put("mp.messaging.outgoing.events-out.enabled", "true");
    return config;
  }
